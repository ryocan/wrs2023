...
void status_receive(const actionlib_msgs::GoalStatusArray::ConstPtr &status_msg)
{
  ...
  // reach mid
    ...
    // reach shelf
    else if ((input_flag == 0 || input_flag == 3) && status_id == 3 && sta == 0)
    {
      output_flag.data = 1;
      sta = 1;
      flag_pub.publish(output_flag);
  ...
  // go mid to go back
  if (input_flag == 1 && (status_id == 0 || status_id == 3) && sta != 4 && sta != 0 && sta != 8 && sta != 100 && sta != 99)
  {
    if (sta2 == 6)
    {
    }
  ...
  // reach back
  if (input_flag == 1 && status_id == 3 && sta == 99)
  {
    output_flag.data = 0;
    sta = 100;
    flag_pub.publish(output_flag);
