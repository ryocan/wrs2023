...
const float thresh_dist = 0.05;  //Threshold Distance between 2 consecutive 'Non-infinity' points to consider them different clusters
...
*/
  ...
  resY.clear();
   ...
   //  cout << numOfPoints.front() << " Num first" << endl;
    ...
    // x_point=(x_first-x_last)/2;
    // y_point=(y_first-y_last)/2;
...
////////
  ...
  for(int j=0;j<numOfPoints.size();j++){
   ...
   // }
    ...
    double x_point = (x1+x2)/2;
    double y_point = (y1+y2)/2;
   ...
   // findAngle(m1,m2);
    ...
    x_pp.push_back(x_point);
    y_pp.push_back(y_point);
...
int main(int argc, char** argv){
  ...
  pub = n.advertise<fcsc::Fin>("fin_data", 10);
