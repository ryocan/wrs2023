...
std_msgs::Int8 input_flag;
std_msgs::Int8 output_flag;
int avoid_flag = 0;
int sta = 0; //0->goalに着く前　1->goalに着いた 3-> 陳列中 4->陳列終了でスタート位置に戻る 5->回避中 6->回避する場所に到着 
...
void status_receive(const actionlib_msgs::GoalStatusArray::ConstPtr& status_msg){
  ...
  if(avoid_flag == 0){//障害物判定0=いない
    //start
    if(input_flag.data == -1 && status_id == 0 && sta == 0){//待機
      output_flag.data = 0;
      sta = 0;
    }

    //go shelf
    if(input_flag.data == 0 && status_id == 0 && sta == 0 &&test==0){//start
  /*    goal.pose.position.x = (shelf_x - origin_x)*resolution;
      goal.pose.position.y = (shelf_y - origin_y)*resolution;*/

goal.pose.position.x = 1.50;
goal.pose.position.y = 1.50;
      ...
      test =1;
      output_flag.data = 0;//0=arm停止
      sta = 0;//棚につく前
    }

    //reach shelf
    if(input_flag.data == 0 && status_id == 3 && sta == 0){//棚についた
      output_flag.data = 1;//作業可能
      sta = 1;//ついた
    }

    //chinretsu
    if(input_flag.data == 2 && status_id == 0 && sta == 1){
      output_flag.data = 1;//
      sta = 3;//陳列中
    }

    //go shelf during going back  
    if(sta == 6){//回避場所にいてかつ障害物なしで棚前に戻る
      goal.pose.position.x = (shelf_x - origin_x)*resolution;
      goal.pose.position.y = (shelf_y - origin_y)*resolution;
      // goal.pose.orientation.z = float(1/sqrt((double)2));
      // goal.pose.orientation.w = float(1/sqrt((double)2));
      ...
      output_flag.data = 0; 
      sta = 0;//棚前に移動中　migireach shelfに行く
    }

  }

  else if(avoid_flag == 1){//障害物がいる
    //kaihi recuest
    if(sta = 3 && avoid_flag == 1){//sta=3陳列中
      output_flag.data = 2;//回避したい
      sta = 3;//陳列中
    }

    if(input_flag.data == 3 && sta == 3){//input=3回避しても良い
      output_flag.data = 0;//作業不可
      sta = 5;//回避中
    }

    //go back
    if(status_id == 0 && sta == 5){//愛費場所の設定
      goal.pose.position.x = (avo_x - origin_x)*resolution;
      goal.pose.position.y = (avo_y - origin_y)*resolution;
      // goal.pose.orientation.z = float(1/sqrt((double)2));
      // goal.pose.orientation.w = float(1/sqrt((double)2));
      goal.pose.orientation.z = 0;
      goal.pose.orientation.w = 1;
    ...
    //reach kaihi
    if(status_id == 3 && sta == 5){//到着
      output_flag.data = 0;
      sta = 6;//回避する場所についた
    }

    //go shelf from back
    /*if(status_id == 0 && sta == 6){//一旦無視
      goal.pose.position.x = (shelf_x - origin_x)*resolution;
      goal.pose.position.y = (shelf_y - origin_y)*resolution;
      // goal.pose.orientation.z = float(1/sqrt((double)2));
      // goal.pose.orientation.w = float(1/sqrt((double)2));
...
/*    if(status_id == 6 && sta == 3){
  ...
  //go backバックヤードに変える
  if(input_flag.data == 1 && status_id == 0){//input1 backヤードに戻る　id=0動いていない
    ...
    output_flag.data = 0;//作業負荷
    sta = 4;//作業終了でスタート位置に戻る
  }

    //reach back
  if(input_flag.data == 1 && status_id == 3 && sta == 4){
    output_flag.data = 0;
    sta = 4;
  }  
